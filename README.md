...
### 5. 🪂 물리 효과 및 중력 모드 (Physics & Gravity Mode)
- **Matter.js 2D 물리 엔진** 라이브러리를 활용해 고양이 이미지들의 리얼한 충돌 및 낙하 처리를 시뮬레이션합니다.
- 하단 푸터의 **중력 모드 토글**을 켜면 화면 속 고양이들이 중력 효과를 받아 우수수 떨어지고 튕기며 바닥에 쌓입니다.
- 중력 모드 ON 상태에서 고양이를 클릭해 지우면, 위에 얹혀 있던 고양이들이 무너지며 아래 빈 공간으로 떨어지는 동적인 물리 상호작용이 일어납니다.
...
| **Physics Engine** | Matter.js | 2D 물리 연산, 충돌 감지 및 중력 모드 시뮬레이션 |
